...
const FurnitureDetail = () => {
  ...
  const videoUrl = furniture?.furnitureVideo
    ...
    : null;
  const furnitureTitle = `${furniture?.furnitureTitle ?? "Furniture"} — Monolith`;
  const furnitureDescription =
    furniture?.furnitureDesc?.slice(0, 160) ??
    "Discover premium furniture and designer pieces at Monolith.";
  const furnitureImage = furniture?.furnitureImages?.[0]
  ...
  const furnitureCanonical = `https://monolith.com/furniture/detail?id=${furnitureId ?? ""}`;
