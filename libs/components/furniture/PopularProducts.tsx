...
const PopularProducts = () => {
  ...
  const getImage = (f: Furniture) =>
    f.furnitureImages?.[0]
      ? `${REACT_APP_API_URL}/${f.furnitureImages[0]}`
      : DEFAULT_IMAGE;
    ...
    <Stack className="popular-products-section">
      ...
      >
        ...
        <Link href="/furniture" style={{ textDecoration: "none" }}>
          ...
          >
            <Typography className="popular-view-all-text">
              View All product
            </Typography>
            <Box
              component="img"
      ...
      <Stack direction="row" gap="24px" className="popular-products-content">
        ...
        <Box
          className="popular-featured-card"
          onClick={() => featured && handleClick(featured._id)}
          sx={{ cursor: featured ? "pointer" : "default" }}
        >
          <img
            src={featured ? getImage(featured) : DEFAULT_IMAGE}
            alt={featured?.furnitureTitle ?? "Featured"}
            className="popular-featured-img"
          />

          {/* Hotspot dots */}
          <Box
            className="popular-hotspot"
            sx={{ left: "381px", top: "329px" }}
          />
          <Box
            className="popular-hotspot"
            sx={{ left: "124px", top: "329px" }}
          />
          <Box
            className="popular-hotspot"
            sx={{ left: "454px", top: "483px" }}
          />

          {/* Info popup */}
          <Stack
            className="popular-featured-popup"
            gap="24px"
            alignItems="center"
          >
            ...
            <Box
              className="popular-buy-btn"
              onClick={(e) => {
                e.stopPropagation();
                featured && handleClick(featured._id);
              }}
            >
              <Typography className="popular-buy-btn-text">Shop Now</Typography>
              <ArrowOutwardRoundedIcon
                className="popular-buy-btn-icon"
                sx={{ fontSize: 16 }}
              />
            </Box>
          </Stack>
        ...
        >
          {(gridItems.length > 0 ? gridItems : Array(4).fill(null)).map(
            (item: Furniture | null, idx) => {
              const image = item ? getImage(item) : DEFAULT_IMAGE;
              const title =
                item?.furnitureTitle ?? "Perfect Bed for a Restorative";
              const price = item?.furniturePrice ?? 40.5;
              const hasDiscount = item && item.furnitureDiscount > 0;
              const originalPrice = hasDiscount
                ? price / (1 - item!.furnitureDiscount / 100)
                : null;
                ...
                >
                  <Box className="popular-grid-img">
                    {item?.furnitureBestseller && (
                      <Box className="popular-grid-badge bestseller">
                        Best seller
                      </Box>
                    )}
                  ...
                  <Stack gap="4px">
                    <Typography className="popular-grid-title">
                      {title}
                    </Typography>
                    <Stack direction="row" alignItems="center" gap="8px">
                      <Typography className="popular-grid-price">
                        ${price.toFixed(2)}
                      </Typography>
                      {originalPrice && (
                        <Typography className="popular-grid-price-old">
