...
### Priority 1 — Wire existing backend to frontend (data integration)
These features have a backend implementation; only the frontend query/mutation wiring is missing.

- **In-app notification bell** — `Notification` schema + `notification.resolver.ts` exist; frontend bell UI not wired to any query
- **Admin: notice/FAQ/inquiry management** — `Notice.model.ts` schema exists; admin UI panels exist (`FaqList`, `InquiryList`, `NoticeList`); resolver may be missing or incomplete — verify before wiring
- **Search** — text index on `furnitureTitle` + `furnitureDesc` exists; `FurnituresInquiry.search` field accepts text; no search input UI or query wiring on /furniture page

### Priority 2 — Backend + frontend both needed (new features)
These require new backend modules and resolver work before frontend can be wired.

- **Order management** — orders collection not yet in schemas; no resolver; checkout UI exists but submits nowhere
- **Address management** — `MyPage > ManageAddress` UI exists; no backend model or resolver
- **Coupon system** — `MyPage > MyCoupons` UI exists; no backend model or resolver

### Priority 3 — Frontend-only enhancements
Backend not needed or already supports it.

- **Mobile SCSS** — only `scss/pc/` implemented; no `scss/mobile/` folder; all pages currently desktop-only
- **3D/AR model viewer** — `furniture3DModel` field exists on Furniture schema and DTO; no viewer component on detail page
- **Email campaign content** — newsletter subscribe/unsubscribe fully wired; actual campaign emails (batch sends) not implemented

### Priority 4 — Deferred / out of scope for now
- **Stripe / real payment** — checkout UI is static form only; Stripe integration deferred indefinitely
- **Member-to-member messaging** — no schema, no resolver, no UI
- **PWA / mobile app** — not planned
