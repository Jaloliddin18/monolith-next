...
- `libs/components/cs/FaqPage.tsx`
- `libs/components/cs/TermsPage.tsx`
- `libs/config.ts`
- `libs/types/notification/notification.ts`
- `pages/_admin/cs/notice.tsx`
- `pages/_app.tsx`

Backend changes (monolith-api):

- `apps/monolith-api/src/components/notice/notice.service.ts`
...
- `apps/monolith-api/src/libs/dto/notification/app-notification.ts`
- `apps/monolith-api/src/schemas/Notification.model.ts`
- `apps/monolith-api/src/socket/socket.gateway.ts`
- `apps/monolith-api/src/socket/socket.module.ts`

New or changed frontend/backend behavior:

- Removed the extra Apollo websocket client; the app now keeps a single shared websocket created in `_app.tsx`.
- Chat now listens to socket events with `addEventListener` instead of assigning `socket.onmessage`, so chat and navbar notifications can coexist on the same socket.
- Chat avatar rendering now normalizes local asset paths and uploaded API paths through `resolveAssetUrl`, fixing broken profile photos for other users in live chat.
- Chat online-member counts now come from unique authenticated member IDs on the backend instead of raw socket connections, fixing doubled counts caused by duplicate websocket sessions per browser.
- Notice fan-out notifications now trigger for `FAQ`, `TERMS`, and `ANNOUNCEMENT` notices whenever they become `ACTIVE`.
- Notification records now persist `noticeCategory`, allowing the frontend navbar to route users to `/cs/faq`, `/cs/terms`, or announcement detail correctly.
- Navbar unread counts now update in real time from websocket `notification` events; opening the drawer also refetches the live list when new notifications arrive.
- FAQ and Terms pages now support `?noticeId=...` deep links so notification clicks open the relevant FAQ item or Terms section directly.
- Newsletter subscription now normalizes email addresses with `trim().toLowerCase()` and safely reactivates duplicate inactive subscribers.

Functional outcomes:

- Other users' profile photos render correctly in the group chat when the stored image path is local or uploaded.
- Online-member badges in chat no longer double-count one browser session as multiple active members.
- Admin-published FAQs, terms, and announcements now produce in-app notifications for active members.
- Navbar notifications arrive immediately over websocket instead of waiting for the 30-second unread-count poll.
- Newsletter signup is resilient to casing/spacing issues and duplicate reactivation flows.

Validation run:

- `monolith-next`: `yarn tsc --noEmit` passed.
- `monolith-api`: `yarn tsc --noEmit --incremental false` passed.
...
- `pages/cs/index.tsx`
- `libs/components/cs/ContactPage.tsx`
- `libs/components/cs/FaqPage.tsx`
- `libs/components/cs/PrivacyPage.tsx`
- `scss/pc/service/service.scss`
- `scss/pc/contact/contact.scss`
- `scss/pc/contact/faq.scss`
- `scss/pc/contact/privacy.scss`
- `scss/pc/contact/terms.scss`
- `pages/_admin/cs/notice.tsx`
- `pages/_admin/cs/inquiry.tsx`
- `pages/_admin/cs/faq.tsx` (repurposed to redirect to unified notices page)
- `libs/components/admin/AdminMenuList.tsx`
- `libs/components/Top.tsx`
- `apollo/user/query.ts`
- `apollo/user/mutation.ts`
- `libs/types/notification/notification.ts`

New frontend GraphQL operations added:
...
- Added `getNoticeById(noticeId: String): Notice` query in Notice resolver/service.
- Added user notification APIs: `getMyNotifications`, `getUnreadCount`, `markNotificationRead`, `markAllNotificationsRead`.
- Added `noticeId` field to `Notification` schema and DTO.
- Extended Notification module to register Notification model.
- Added announcement fan-out notifications when:
  - `createNoticeByAdmin` creates an `ACTIVE` notice.
  - `updateNoticeByAdmin` transitions notice status from `HOLD` to `ACTIVE`.

Functional outcomes:

- CS system is fully wired to real backend data for FAQs, notices, and user inquiries.
- Navbar notification bell + announcement drawer is implemented with unread count and read actions.
- Privacy page supports announcement detail mode via `?announcementId=...` while preserving default privacy content.
- Admin CS navigation is consolidated to **Announcements** + **1:1 Inqury** (FAQ management removed from active admin nav flow).
- Announcement creation/activation now triggers notifications for all active members.

Validation run:

- `monolith-next`: `yarn tsc --noEmit` passed.
- `monolith` backend: `yarn build` passed.
...
### Stats (admin-only, read-only)

- Resolver: `components/stats/stats.resolver.ts`
- Service: `components/stats/stats.service.ts`
- Module: `components/stats/stats.module.ts`
- DTO: `libs/dto/stats/stats.ts` — AdminStats, CategoryCount, RoomCount, MonthCount
- No schema (reads from Member, Furniture, BoardArticle, Subscriber collections via aggregation)
- Single query: `getAdminStats` guarded by RolesGuard (ADMIN)
...
| getAdminStats              | —                         | AdminStats    | RolesGuard (ADMIN)         |
...
GetAllMembersByAdmin, GetAllFurnituresByAdmin, GetAllBoardArticlesByAdmin, GetComments, GetAdminStats
...
- Recharts in Next.js: never dynamically import individual recharts components — import all needed components from one `async () => { const { X, Y } = await import('recharts'); return function Component() {...} }` call, otherwise Cell/Legend/Tooltip lose type identity and props don't apply
- Admin dashboard charts use recharts loaded via two single-dynamic-import wrapper components (GrowthChart, CategoryPieChart) defined in `pages/_admin/index.tsx`
- MUI v7 Grid removed from admin pages — use CSS grid (`display: grid`, `gridTemplateColumns`) to avoid MUI v7 Grid API breaking changes (`item`/`xs` props removed)
...
- All 9 modules implemented: Member, Furniture, BoardArticle, Comment, Follow, Like, View, Notification, Stats
...
- Stats module: getAdminStats returns totals, top 5 viewed/liked furnitures, top 5 designers, last 10 members, furniture by category/room, member growth last 6 months
...
- Admin panel: member/furniture/article management wired to live API
- Admin dashboard: live stats cards, member growth line chart, category pie chart, top viewed/liked products tables, recent members table — all from getAdminStats
- Admin layout: dark sidebar (#1C1A17) with Dashboard/Members/Furniture/Articles/CS nav, sticky topbar with page title
- Newsletter: subscribe/unsubscribe pages wired
- Live chat: WebSocket connected
- i18n: next-i18next configured (en, kr, ru)
- recharts installed (v3); DM Sans + Playfair Display fonts loaded in \_document.tsx

---

## Pending / Next

1. Mobile layout: homepage, furniture listing, community, and mypage pages still need mobile branches (useDeviceDetect pattern; Top and Footer already done).
2. Notification bell wiring — in-app notifications not yet wired to frontend bell icon.
3. Admin CS pages — notice/faq/inquiry wiring verification (backend pending for Notice).
4. Order/address/coupon backend integration not yet implemented.
5. Admin panel mobile view not handled — admin is desktop-only by design but no explicit guard exists.
6. Verify getAdminStats query in GraphQL playground after backend restart with new StatsModule.
